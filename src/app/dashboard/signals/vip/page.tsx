...
export default function VIPSignalsPage() {
  ...
  const loadSignals = async () => {
    try {
      setLoading(true)
      console.log('Loading VIP signals for user:', user)
      const data = await getSignalsByCategory('vip')
      console.log('VIP signals loaded:', data)
      setSignals(data)
    } catch (error) {
      console.error('Error loading VIP signals:', error)
...
Status: ${signal.status.replace('_', ' ').toUpperCase()}${signal.result ? `\nResult: ${signal.result} pips` : ''}`
  ...
  const hasVIPAccess = user?.role === 'vip' || user?.isAdmin
  ...
  // Show protected content if user doesn't have VIP access
  if (!hasVIPAccess) {
    return (
      <div className="container mx-auto px-4 py-8">
          ...
          <Card className="border-red-200 dark:border-red-800/30">
            <CardContent className="p-12">
              ...
              <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-4">
                VIP Signals Protected
              </h1>
              <p className="text-lg text-slate-600 dark:text-slate-400 mb-6">
                This content is exclusively available to VIP members. Upgrade your account to access premium trading signals with higher success rates and advanced analysis.
              </p>
              
              <div className="bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-lg p-6 mb-6 border border-red-200 dark:border-red-800/30">
                <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center justify-center space-x-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-slate-700 dark:text-slate-300">Exclusive VIP signals</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Star className="w-5 h-5 text-red-500" />
                    <span className="text-slate-700 dark:text-slate-300">Higher success rates</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Star className="w-5 h-5 text-red-500" />
                    <span className="text-slate-700 dark:text-slate-300">Advanced analysis</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Star className="w-5 h-5 text-red-500" />
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button asChild className="bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white">
                  <Link href="/pricing">
                    Upgrade to VIP
                  </Link>
                </Button>
                <Button asChild variant="outline">
                  <Link href="/dashboard/signals/free">
                    View Free Signals
                  </Link>
                </Button>
              </div>
        ...
        <CardHeader>
          <div className="text-center">
            ...
            <CardDescription className="text-slate-600 dark:text-slate-400 mt-2">
              Exclusive premium trading signals for VIP members
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {signals.length === 0 ? (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold text-slate-600 dark:text-slate-400 mb-2">
                  No VIP signals available
                </h3>
                <p className="text-slate-500 dark:text-slate-500 text-center">
                  Check back later for new VIP trading signals.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          signals.filter(signal => signal && signal.id).map((signal) => {
            ...
            return (
              <Card key={signal.id} className="group hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border-2 border-red-200 dark:border-red-800/30 hover:border-red-400 dark:hover:border-red-500 bg-gradient-to-br from-white to-red-50/30 dark:from-black dark:to-red-900/10">
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <Badge className="bg-gradient-to-r from-red-600 to-orange-500 text-white text-sm font-bold shadow-lg">
                  ...
                  {/* Enhanced VIP Analysis Notes */}
                  {signal.notes && (
                    <div className="mt-4 p-3 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-lg border border-red-200 dark:border-red-800/30">
                      <div className="flex items-start space-x-2">
                        <Star className="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5 flex-shrink-0" />
                        <div>
                          <p className="text-sm font-semibold text-red-700 dark:text-red-300 mb-1">VIP Analysis</p>
